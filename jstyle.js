...
(function() {
    ...
    var A;
    var B;
    z = window.jStyle = window.cs$ = function(a, b, c) {
        if (a && a.nodeType === 9) {
        ...
        var d = [];
        if (typeof(b) == "string") {
            if (" " > b || typeof(c) == "undefined") return false;
            var e = b.split(".");
            var f = [];
            if (typeof(a) == "string") d = z.find(a);
            ...
            d = z.matches(h.filter, d);
            for (var i = 0; i < d.length; i++) {
                f.length = 0;
                f = f.concat(e);
                if (!d[i].getAttribute("jstyle")) d[i].setAttribute("jstyle", "{}");
                g = z.getOrCreateElementJStyle(d[i]);
                x(g, b);
                var k = f.pop();
                var l = y(g, f.join("."));
                if (typeof(c) == "object" && !isArray(c)) {
                    p(l[k], c)
                } else {
                    l[k] = c
                }
                h.build_parameters(y(g, h.styleName), d[i]);
                h.render(y(g, h.styleName), d[i])
            }
        }
        delete d;
        return z.styles
    ...
    z.getStyle = function(a) {
        ...
        var c;
        for (var i = 0; i < z.styles.length; i++) {
            c = new RegExp("^" + z.styles[i].styleName);
            if (c.test(a)) {
                b = z.styles[i];
                break
            }
    ...
    z.addStyle = function(a) {
        ...
        p(d, b);
        z.styles.push(d);
        x(z.styles, b.styleName);
        var e;
        var f = b.styleName.split(".");
        var g = f.pop();
        e = y(z.styles, f.join("."));
        e[g] = d;
        return true
